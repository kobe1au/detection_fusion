...
def test_raw_config_schema_rejects_unknown_fields() -> None:
    with pytest.raises(
        ValueError,
        match="Unsupported fusion configuration keys",
    ):
        _validate_raw_config_schema(
            {
                "fusion": {
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"model": {"api_encoder": {"unregistered_option": 1}}},
            r"model\.api_encoder",
        ),
        (
            {"model": {"manifest_encoder": {"enabled": True}}},
            r"model\.manifest_encoder",
        ),
        (
            {"loss": {"tmc": {"unregistered_option": 1}}},
            r"loss\.tmc",
        ),
        (
            {"data": {"max_graph_nodes_per_sample": 1024}},
            "Unsupported data configuration keys",
        ),
    ],
)
def test_raw_config_schema_rejects_nested_or_duplicate_budget_fields(
    payload: dict,
    message: str,
) -> None:
    with pytest.raises(ValueError, match=message):
        _validate_raw_config_schema(payload)
