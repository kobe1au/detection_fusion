...
def _write_current_pt(
    ...
    *,
    ...
    save_current_pt(path, _dex_payload(marker), manifest_dim=16)
...
def test_later_pt_provenance_mismatch_is_fatal_not_dummy(tmp_path: Path):
    """Feature-dimension inference sees a valid first PT; the second must still fail."""
    first_sid = _sid("1")
    second_sid = _sid("2")
    pt_dir = tmp_path / "pts"
    pt_dir.mkdir()
    ...
    dataset = _dataset(pt_dir, csv_path)
    first = dataset[0]
    assert first.sid == first_sid
    assert first.is_dummy is False
...
def test_renamed_pt_is_rejected_by_migration_and_dataset(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="direct_build_meta.sha256 mismatch"):
