...
def test_formal_registry_is_exactly_the_run_catalog() -> None:
    assert len(BASELINE_PATHS) == 13
    assert len(REGISTERED_BASELINES) == 13
    observed = {
...
def test_identity_matrix_rejects_mislabeled_or_invalid_lifecycle(
    path: Path,
    mutator,
    message: str,
) -> None:
...
def test_identity_matrix_rejects_non_registered_method() -> None:
    ...
    with pytest.raises(ValueError, match="13 registered"):
        validate_registered_baseline_config(cfg)
