...
if len(REGISTERED_BASELINES) != 13:
    raise RuntimeError("The formal baseline registry must contain exactly 13 methods")
...
def _reject_any_enabled(cfg: Mapping[str, Any]) -> None:
    ...
    if active:
        raise ValueError(
            "The registered baseline runner permits only direct training and "
            f"evaluation with fixed argmax classification; active={active}"
        )
...
def validate_registered_baseline_config(
    ...
    if protocol_id not in REGISTERED_BASELINES:
        raise ValueError(
            "The baseline runner accepts only the 13 registered method "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Train and evaluate one of the 13 registered comparison baselines."
        )
    )
    parser.add_argument(
        "--config",
