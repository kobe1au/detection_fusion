...
def test_collector_rejects_retired_proposed_method_sections(
    tmp_path: Path,
) -> None:
    payload = _baseline_summary()
    payload["stage_b_training"] = {"enabled": True}
    _write(tmp_path / "summary.yaml", payload)
    with pytest.raises(ValueError, match="retired proposed-method"):
