...
def _care_stage_a_loss(
    ...
    incompatible = {
        ...
        "evidential_loss_weight": float(
            loss_cfg.get("evidential_loss_weight", 0.0)
        ),
    }
    nonzero = {name: value for name, value in incompatible.items() if value != 0.0}
    if nonzero:
        raise ValueError(
            "CARE Stage A is exactly the equal-weight four-path clean CE; "
            f"set incompatible generic weights to zero: {nonzero}"
        )
    label_smoothing = float(loss_cfg.get("label_smoothing", 0.0))
    ...
    path_logits = outputs.get("care_path_logits")
    if not isinstance(path_logits, dict) or set(path_logits) != set(PATH_NAMES):
        raise ValueError(
            "CARE Stage A requires care_path_logits for AGM, AG, AM, and GM"
        )
    path_available = outputs.get("care_path_available")
    if (
    ...
    for path_index, path_name in enumerate(PATH_NAMES):
        ...
        if (
            ...
            or current_logits.shape != logits.shape
        ):
            raise ValueError(
                f"CARE {path_name.upper()} logits must match primary logits shape"
            )
    ...
    active_matrix = torch.stack(active_paths)
    total = (
        torch.stack(per_path_loss) * active_matrix
    ).sum() / active_matrix.sum().clamp_min(1.0)
    diagnostics["loss"] = total.detach()
    diagnostics["care_stage_a_ce"] = total.detach()
    diagnostics["care_active_path_count"] = active_matrix.sum().detach()
    return total, diagnostics
...
def compute_robust_loss(
    ...
    *,
    ...
    if objective == CARE_STAGE_A_OBJECTIVE:
        total, parts = _care_stage_a_loss(
            logits,
            labels,
            extra,
            loss_cfg,
    ...
    if objective not in {"standard", "default", "generic"}:
        raise ValueError(
            "loss.objective must be care_stage_a_clean, "
            "standard, tmc, or ecml; "
            f"got {objective!r}"
        )
