...
def validate_care_config(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    if configured_mechanisms != CARE_EVAL_MECHANISMS:
        raise ValueError(
            "CARE formal evaluation must contain exactly the frozen ordered "
            f"mechanisms {list(CARE_EVAL_MECHANISMS)}"
        )
    ...
    if calibration_cfg not in ({}, {"enabled": False}):
        raise ValueError(
            "CARE resolved config must not retain the retired post-hoc "
            f"calibration stack: {sorted(calibration_cfg)}"
        )
    fusion_cfg = cfg.get("fusion", {}) or {}
    if fusion_cfg not in ({}, {"mode": "care_droid"}):
        raise ValueError(
            "CARE resolved config must not retain legacy fusion/I1/I2 keys: "
            f"{sorted(fusion_cfg)}"
        )
...
def _care_upstream_runtime_payload(
    ...
    return {
        ...
        "data_input_semantics": {
            ...
            "label_map": _json_compatible(data_cfg.get("label_map")),
        },
        "numeric_inference": {
...
def _run_care_eval_only(
    ...
    if (
        ...
        raise ValueError(
            ...
            "pipeline; intermediate/older checkpoints are unsupported"
        )
