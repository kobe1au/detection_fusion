...
from fusion.losses import REMOVED_LOSS_CONFIG_KEYS
...
def _canonicalize_fixed_classification_config(cfg: dict) -> dict:
    ...
    if unknown:
        raise ValueError(
            ...
            f"remove retired threshold-fitting keys: {unknown}"
        )
    ...
    if enabled:
        raise ValueError(
            "Classification-threshold fitting was removed; current methods "
            "require classification_threshold.enabled=false"
        )
...
def _apply_resolved_config_defaults(cfg: dict) -> dict:
    ...
    if protocol_id == "care_droid_v1":
        ...
        if retired_model_keys:
            raise ValueError(
                "CARE-Droid rejects retired model settings: "
                f"{retired_model_keys}"
            )
        if (
        ...
        resolved.pop("adaptive_fusion", None)
...
def _reject_removed_config_keys(cfg: dict) -> dict:
    ...
    if "robust" in cfg:
        raise ValueError(
            "The ambiguous legacy section 'robust' was removed"
        )
    retired_top_level = sorted(
        set(cfg) & {"adaptive_fusion", "paired_training", "stage_b"}
    )
    if retired_top_level:
        raise ValueError(
            "Retired proposed-method sections are unsupported: "
            f"{retired_top_level}"
        )
    ...
    if isinstance(train_cfg, dict):
        ...
        if removed_train:
            raise ValueError(
                "Removed Stage-A tuning settings are unsupported: "
                f"{removed_train}"
            )
    data_cfg = cfg.get("data")
    if isinstance(data_cfg, dict) and "graph_semantic_source" in data_cfg:
        raise ValueError(
            "Removed key data.graph_semantic_source is unsupported"
        )
    ...
    if isinstance(eval_cfg, dict) and "robust_val" in eval_cfg:
        raise ValueError("eval.robust_val was removed")
    fusion_cfg = cfg.get("fusion")
    if (
        isinstance(fusion_cfg, dict)
        and "use_reliability_discount" in fusion_cfg
    ):
        raise ValueError(
            "Removed key fusion.use_reliability_discount is unsupported"
        )
    ...
    if isinstance(calibration_cfg, dict) and "epochs" in calibration_cfg:
        raise ValueError("calibration.epochs was removed")
    return cfg
...
def _normalize_robust_test_protocol(
    ...
    raw_strengths = (
        ...
        else [eval_cfg.get("perturb_strength", 0.5)]
    )
