...
from paper.baselines.common import (
    DEFAULT_VALIDATION_ROLE_ASSIGNMENT,
    binary_metrics,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=str(DEFAULT_VALIDATION_ROLE_ASSIGNMENT),
        help=(
            "Immutable v2 validation role assignment shared with the main "
            "method; only its model_selection role is used for checkpointing."
        ),
    )
    ...
    selection_indices, validation_split = validation_selection_indices(
        val_frame,
        validation_csv_path=args.val_csv,
        role_assignment_path=args.validation_role_assignment,
    )
